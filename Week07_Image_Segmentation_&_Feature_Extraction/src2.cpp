...
{
	int SumX = 0;	//동공 영역의 X좌표 위치 누적합
	int SumY = 0;	//동공 영역의 Y좌표 위치 누적합
	int cnt = 0;	//동공 영역의 픽셀 개수
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if (Img[i * W + j] == 0) {	//동공영역이면
				SumX += j;
				SumY += i;
				cnt++;
			}
	...
	if (cnt == 0)	//cnt = 0일때의 예외처리
		cnt = 1;

	*Cx = SumX / cnt;	//X좌표에 대한 무게중심
	*Cy = SumY / cnt;	//Y좌표에 대한 무게중심
	//printf("%d, %d", Cx, Cy);
}
...
{
	for (int i = 0; i < W; i++)	//horizontal line
		Img[Cy * W + i] = 255;
	for (int i = 0; i < H; i++)	//vertical line
		Img[i * W + Cx] = 255;
}

void DrawRectOutline(BYTE* Img, int W, int H, int LUX, int LUY, int RDX, int RDY)
{
...
int main()
{
	...
	InverseImage(Temp, Output, W, H);
	m_BlobColoring(Output, H, W);

	int Cx, Cy;
	Obtain2DCenter(Output, W, H, &Cx, &Cy);	//이진영상의 무게중심
	int LUX, LUY, RDX, RDY;
	...
	SaveBMPFile(hf, hInfo, hRGB, W, H, Image, "output_pupil1_cross_bound.bmp");
