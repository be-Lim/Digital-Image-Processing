...
이 저장소는 **디지털 영상 처리(Digital Image Processing)**의 핵심 이론을 학습하고, 라이브러리에 의존하지 않는 **로우레벨(Low-level) 알고리즘 구현 능력**을 기르기 위한 기록장입니다. 
...
| :---: | :--- | :--- | :---: |
| **Week 03** | **Point Processing (점 처리)** | BMP 바이너리 구조 분석, 영상 밝기 조절 및 반전 | [📂 Go](Week03_Digital_Image_Files/) |
...
1. `src.cpp` : 알고리즘이 구현된 메인 소스 코드 (핵심 로직 주석 포함)
2. `README.md` : 해당 주차의 **핵심 수학적/공학적 이론 요약** 및 **실제 실행 결과(Before/After 이미지)** 증명
